...
# Получение Telegram токена и ID администраторов из .env файла
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN")
ADMIN_TELEGRAM_IDS = [int(admin_id.strip()) for admin_id in os.getenv("ADMIN_TELEGRAM_IDS").split(",")]
...
async def choice_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "🚚 Заказать транспорт":
        ...
        if not user_orders:
            ...
            for order in user_orders:
                message += (
                    ...
                    f"• *Откуда:* {order['order_details']['origin']}\n"
                    f"• *Куда:* {order['order_details']['destination']}\n"
                    f"• *Дата и время:* {order['order_details']['date_time']}\n"
                    f"• *Тип груза:* {order['order_details']['cargo_info']}\n\n"
                )
            await update.message.reply_text(message, parse_mode='Markdown')
        await update.message.reply_text("Вы вернулись в главное меню.", reply_markup=create_main_menu())
        return CHOOSING
        ...
        if update.effective_user.id in ADMIN_TELEGRAM_IDS:
            keyboard = [
                [KeyboardButton("/notify"), KeyboardButton("/view_orders")],
                [KeyboardButton("Назад")]
            ]
            await update.message.reply_text(
                "🔧 *Админ-меню:*\n\nВыберите действие:",
...
async def view_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not orders:
        ...
        for order_id, details in orders.items():
            message += (
                f"*ID заказа:* {order_id}\n"
                f"• *Откуда:* {details['order_details']['origin']}\n"
                f"• *Куда:* {details['order_details']['destination']}\n"
                f"• *Дата и время:* {details['order_details']['date_time']}\n"
                f"• *Тип груза:* {details['order_details']['cargo_info']}\n\n"
            )
        await update.message.reply_text(message, parse_mode='Markdown')

# Обработка обратной связи для завершения диалога
